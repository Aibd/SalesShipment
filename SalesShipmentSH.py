...
def delete_jdy_data(jdy_api, kdata):
    ...
    for data in kdata:
        ...
        query_data["filter"]["cond"][2]["value"] = '希肤上海'
...
def SalesShipmentSH():
    ...
    # 获取金蝶销售出库数据
    with MSSqlDBManager('希肤上海') as msmanager:
    ...
    # 比对删除数据，删除被同步端数据
    with SqliteDBManager('SalesShipment.db') as sqlitemanager:
        query_sql = Common.deleted_query_sql.format(company='希肤上海')
    ...
    with SqliteDBManager('SalesShipment.db') as sqlitemanager:
        diff_sql = Common.differences_sql.format(company='希肤上海')
